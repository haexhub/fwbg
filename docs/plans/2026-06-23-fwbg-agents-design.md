...
**Datum**: 2026-06-23
**Status**: M0+M1+M2+M3+M4+M5a+M5b+M5c+M6a implemented; M6b (Paper-Analyst + Promote-Live) next, M5d (PluginPlanner/Implementer split) parked
...
| M6b — Paper-Analyst + Promote-Live | pending | Scope sketched in `docs/plans/2026-06-24-fwbg-agents-m6.md` "Next Milestones": `data/criteria/paper/<class>.yaml` hand-curated (looser than backtest, [[feedback-no-data-derived-thresholds]]), `agents/paper_analyst.py` (pydantic-ai discriminated-union Promote/Abandon/ContinueObservation with deterministic validator: Promote requires criteria-pass, Abandon requires post_mortem_path, ContinueObservation auto-flags stale=True when `days_in_paper > paper_phase_target_days`), `orchestrator/paper_flow.py` writes sidecar + sets `Strategy.metadata.paper_analyst_promote_recommended=True` on Promote (NO transition — analyst never transitions to live), `POST /strategies/{id}/paper-analyze` (manual trigger, BackgroundTasks envelope), `POST /strategies/{id}/promote-live` requires `payload.human_approval=true` AND prior promote-recommendation flag + calls M2's `transition_strategy(paper_trading→live_trading, human_approval=true)`. ~5-6h, agents-only, ~+20 tests. |

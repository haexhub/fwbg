...
**Datum**: 2026-06-23
**Status**: M0+M1+M2+M3+M4+M5a implemented, M5b (PluginAuthor + Evaluator + API) next
...
| M5b — PluginAuthor + Evaluator + API | pending | Scope sketched in `docs/plans/2026-06-24-fwbg-agents-m5a.md` (M5b preview section). Tasks: PluginAuthor agent (LLM, writes `data/plugins/<slug>/v1/{plugin.py, contract.yaml, spec.md}`), PluginEvaluator (deterministic, hand-curated scenario generators, dynamic-imports plugin.py, runs invariants), Migration 0004 (verification_run table + PluginKind string-column extension to all 8 categories), `POST /strategies/{id}/author-plugin`, `POST /plugins/{id}/evaluate`, `GET /plugins/{id}/verification-runs`, `scripts/m5_smoke.py` end-to-end. |

...
**Datum**: 2026-06-23
**Status**: M0+M1+M2+M3 implemented, M4 next
...
| M4 — Researcher + Translator | in progress 2026-06-24 (Tasks 1-6 of 8 done) | Plan at `docs/plans/2026-06-24-fwbg-agents-m4.md`. Latest commit `ed2b59a` in `~/Projekte/fwbg-agents/`. Done: `prior_art` deterministic tag-similarity (`1e9707e`), `ResearcherHypothesis` schema + `validate_hypothesis` + `generate_slug` (`4d5041a`), Tavily client + quota tracking via `llm_call(model='tavily-search')` (`3991d4f`), Researcher LLM agent with `lookup_prior_art` + `search_web` tools and hard anti-redundancy gate (`6dd3093`), lightweight `strategy.json` structural validator with hardcoded plugin-slug catalog (`3d699da`), Translator fresh-mode LLM (hypothesis → strategy.json + spec.md, canonical slug enforced) (`35324cf`), Translator reiterate-mode fully deterministic with `parent_strategy_id` lineage + extended `ChangeExit.new_exit_strategy` field on Analyst (`ed2b59a`). 147 tests green (78 baseline + 69 new). **Open in next session**: Task 7 `orchestrator/research_flow.py` + `api/research.py` (`POST /research/brief`, `POST /strategies/{id}/reiterate`, `GET /hypotheses`), Task 8 `scripts/m4_smoke.py` + final verification. Locked decisions: re-iterate via `parent_strategy_id` (not state-machine regression), Tavily quota via convention `model='tavily-search'` (no schema change), strategy.json validation is lightweight structural — full `fwbg.core.config.StrategyConfig` deferred, Anthropic web_search fallback documented but not built. |

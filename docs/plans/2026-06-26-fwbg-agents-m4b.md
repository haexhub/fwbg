...
**Status:** ✓ done 2026-06-26 — five commits `9ad4cc2`, `cce8f28`, `4f46df5`, `9b6b6fc`, `053b144` in `~/Projekte/fwbg-agents/` on branch `feat/m4b-researcher-resilience` (off `develop`, see branch-strategy deviation below). 384 tests green (370 baseline → +14 net new). `scripts/m4_smoke.py` still exits 0 (clean skip, unchanged). `scripts/m4b_smoke.py` printed a literal `[m4b_smoke] PASSED` on 2026-06-27 once `haex-claude-proxy` was actually deployed and reachable (it wasn't at the time the plan was first executed — see deviations below); getting there also surfaced and fixed four real, pre-existing bugs in `haex-claude-proxy` itself (stale pinned CLI version, dropped structured-output tool schema, a concurrent-token-refresh race, and — the big one — the proxy silently dropping every request's system prompt for ~7 weeks).
...
## Acceptance criteria (Definition of Done) — ✓ fully met
...
- `scripts/m4b_smoke.py` ends `[m4b_smoke] PASSED`. ✓ Reached for real on 2026-06-27 against a deployed, working `haex-claude-proxy` — see deviations below for the path to get there.
...
- **Task 5 — live verification scope:** `scripts/m4b_smoke.py` was first verified live against a freshly-migrated local `data/state.db` (user-approved `alembic upgrade head`) through `POST /research/brief` → background task → fan-out of 2 real `Researcher.run()` candidates → real (failed) LLM calls, with `haex-claude-proxy` not yet deployed: both candidates correctly raised on `Connection error`, `ResearcherFanoutExhaustedError` correctly combined both reasons, the `AgentRun` was correctly marked `failed`, and the script correctly exited 1. This validated every code path up to the external LLM boundary.
- **Post-plan: getting to a literal `[m4b_smoke] PASSED` (2026-06-27).** Deploying `haex-claude-proxy` into the `fwbg` stack (new `claude-proxy` service, port `12102`) and completing its OAuth setup surfaced four real, pre-existing bugs in that separate repo, all found, fixed, tested, and shipped (`ghcr.io/haexhub/haex-claude-proxy:latest`) during this session: (1) fresh named Docker volumes are root-owned, hanging `claude auth login` on a non-root user — fixed via an entrypoint chown+privilege-drop; (2) the pinned CLI version (`2.1.121`) was too old for Anthropic's current OAuth UX, hanging forever before the code prompt — bumped to `2.1.191`; (3) the proxy silently dropped every request's `tools`, so pydantic-ai's `output_type=` structured output never reached the model as a real schema — added `final_result` → `--json-schema` translation; (4) concurrent requests sharing one OAuth credential could corrupt `credentials.json` via a refresh-token race — added a per-credential-HOME lock. A **fifth, unrelated and far older bug** (regressed ~7 weeks prior in commit `4d0e1ab`) was also found this way: `/v1/messages` silently dropped the `system` field entirely, despite a comment claiming otherwise — meaning every agent call through this proxy had been running with zero system instructions. Fixing that surfaced one more, smaller fix needed *in this repo*, outside M4b's stated file list: `agents/translator.py` / `agents/prompts/translator.md` — the Translator's `get_known_plugins` tool (a plain, deterministic, side-effect-free function) is now also rendered directly into its system prompt as static `known_plugins_json` context, not only offered as a tool call, since this proxy still doesn't support the multi-turn tool_use ⇄ tool_result loop true function-tool-calling needs (a documented, separate, larger limitation — out of scope for both M4b and today's fixes). With all of that in place, `scripts/m4b_smoke.py` against the real, fixed, OAuth-linked proxy: `[m4b_smoke] PASSED — strategy liquiditysweepmeanrev__forex__001 (id=3)`.

...
class RiskIndicators(BaseIndicator):
    ...
    def compute(
        ...
        **params
        ...
        # === Correlation Features (compute before crash probability) ===
        corr_features = {}
        if compute_correlations:
            corr_features = self._compute_correlation_features(df, features)

        # Merge all features so far for crash probability computation
        all_features = {**features, **corr_features}

        # === Crash Probability ===
        crash_features = self._compute_crash_probability(df, all_features)
        all_features.update(crash_features)

        # Concat all features at once (avoids fragmentation)
        df = pd.concat([df, pd.DataFrame(all_features, index=df.index)], axis=1)
        ...
        shifted = {col: df[col].shift(1) for col in feature_cols}
    ...
    def _compute_crash_probability(
        self, df: pd.DataFrame, features: dict
    ) -> dict:
        """
        ...
        Returns dict of new features (not modifying df directly).
        """
        ...
        # Vol-of-Vol basiert
        if "risk_vol_of_vol_zscore" in features:
            vov_contrib = features["risk_vol_of_vol_zscore"].clip(0, 3) / 3
            crash_score += vov_contrib
            n_components += 1

        # Correlation Decoupling basiert
        if "corr_spx_decoupling" in features:
            decoupling_norm = safe_divide(
                features["corr_spx_decoupling"],
                features["corr_spx_decoupling"].rolling(100).max()
            )
            crash_score += decoupling_norm.clip(0, 1)
            n_components += 1

        # CVaR basiert
        if "risk_cvar_5_100" in features:
            cvar_zscore = safe_divide(
                features["risk_cvar_5_100"] - features["risk_cvar_5_100"].rolling(100).mean(),
                features["risk_cvar_5_100"].rolling(100).std()
            )
        ...
        result = {}
        if n_components > 0:
            crash_prob = crash_score / n_components
        else:
            crash_prob = pd.Series(0.0, index=df.index)

        result["risk_crash_probability"] = crash_prob
        result["risk_crash_prob_change"] = crash_prob - crash_prob.shift(10)
        result["risk_crash_regime"] = (crash_prob > 0.6).astype(int)
    ...
    def _compute_correlation_features(
        ...
        if has_spx:
            ...
            corr_spx_50 = result["corr_spx_50"]
            result["corr_spx_stability"] = corr_spx_50 - corr_spx_50.shift(10)
            result["corr_spx_decoupling"] = abs(result["corr_spx_stability"])
        ...
        if vix_col:
            ...
            for window in [20, 50]:
                result[f"corr_vix_{window}"] = close.rolling(window).corr(vix)

            vix_change = vix.pct_change(10)
            asset_change = close.pct_change(10)
            result["lead_lag_vix"] = vix_change + asset_change

            result["vix_lead_signal"] = vix_change.shift(5) * 100

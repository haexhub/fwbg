...
class TestComputeRetestSignals:
    ...
    def test_departure_prevents_immediate_retest(self):
        """Signal must not fire on breakout bar or immediately after without departure."""
        # range: [96, 104], midpoint=100, range=8
        # entry_bull = midpoint = 100, half_band = 2 → zone [98, 102]
        # departure needs C > 102
        n = 4
        close, rh, rl, gids, bh, bl = self._make_scenario(
            n, 104.0, 96.0,
            [100.0, 105.0, 100.0, 100.0],  # bar0=neutral, bar1=breakout, bar2/3=midpoint
        )
        bh[:] = [False, True, True, True]  # broke_high from bar1
        ...
        # bar1: C=105 > 102 → departed. bar2: C=100 in [98,102] → fires
        assert result["retest_bull"][1] == 0.0  # breakout bar itself
        assert result["retest_bull"][2] == 1.0  # retest after departure

    def test_no_departure_no_signal(self):
        """If price never departs the zone, no retest signal fires."""
        n = 4
        close, rh, rl, gids, bh, bl = self._make_scenario(
            n, 104.0, 96.0,
            [100.0, 101.5, 100.0, 100.0],  # breakout at 101.5 but never > 102
        )
        bh[:] = [False, True, True, True]
        entry_bull = np.full(n, 100.0)
        entry_bear = np.full(n, 100.0)
        half_band = np.full(n, 2.0)  # zone [98, 102], departure needs > 102
    ...
    def test_session_mask_blocks_signal(self):
        ...
        result = compute_retest_signals(
            ...
            group_ids=gids, broke_high_arr=bh, broke_low_arr=bl,
            entry_bull=entry_bull, entry_bear=entry_bear,
            half_band=half_band, n=n, session_mask=session_mask,
        )
    ...
    def test_bear_retest(self):
        """Bear retest: breakout below range, departure, return to zone."""
        n = 4
        ...
        entry_bull = np.full(n, 100.0)
        entry_bear = np.full(n, 100.0)
        half_band = np.full(n, 2.0)  # zone [98, 102]
        ...
        # bar1: C=94 < 98 → departed_bear. bar2: C=100 in zone, < range_high → fires

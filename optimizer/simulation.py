...
def generate_trend_labels(df, lookback=24, threshold_pct=0.3):
    """
    Generiert Trend-Labels OHNE Look-Ahead Bias.

    Statt zu fragen "Hat der Trade gewonnen?" (was Zukunftsdaten braucht),
    fragen wir: "Gab es in der Vergangenheit einen klaren Trend?"

    Das Modell lernt dann: "Bei welchen Feature-Kombinationen gab es
    historisch einen Trend?" - und wir hoffen, dass dieser Trend sich fortsetzt.

    WICHTIG: Dies ist ein fundamentaler Paradigmenwechsel!
    - ALT: Modell lernt "welche Features führen zu Gewinn" (Look-Ahead Bias!)
    - NEU: Modell lernt "welche Features deuten auf Trend-Fortsetzung"

    Args:
        df: DataFrame mit OHLC-Daten und Indikatoren
        lookback: Wie viele Bars zurückschauen für Trend-Bestimmung
        threshold_pct: Minimale Preisänderung in % für Trend-Label

    Returns:
        tuple (targets_long, targets_short) - numpy arrays mit 0/1 Labels
    """
    n = len(df)
    ...
    for i in range(lookback, n):
        # Berechne Return der letzten `lookback` Bars (VERGANGENHEIT, nicht Zukunft!)
        past_return = (closes[i] - closes[i - lookback]) / closes[i - lookback] * 100

        # Long-Trend: Preis ist gestiegen -> wir erwarten Fortsetzung
        if past_return > threshold_pct:
            targets_long[i] = 1

        # Short-Trend: Preis ist gefallen -> wir erwarten Fortsetzung
        elif past_return < -threshold_pct:
            targets_short[i] = 1

    return targets_long, targets_short


def generate_mean_reversion_labels(df, lookback=24, threshold_pct=0.5):
    """
    Generiert Mean-Reversion Labels OHNE Look-Ahead Bias.

    Gegenteil von Trend-Following: Wenn Preis stark gefallen ist,
    erwarten wir Erholung (Long), und umgekehrt.

    Args:
        df: DataFrame mit OHLC-Daten
        lookback: Bars zurückschauen
        threshold_pct: Minimale Preisänderung für Label

    Returns:
        tuple (targets_long, targets_short)
    """
    n = len(df)
    targets_long = np.zeros(n)
    targets_short = np.zeros(n)
    ...
    for i in range(lookback, n):
        ...
        # Mean-Reversion: Nach starkem Fall erwarten wir Erholung
        if past_return < -threshold_pct:
            targets_long[i] = 1

        # Nach starkem Anstieg erwarten wir Korrektur
        elif past_return > threshold_pct:
            targets_short[i] = 1
...
def generate_momentum_labels(df, fast_period=8, slow_period=24):
    """
    Generiert Momentum-Labels basierend auf EMA-Crossover.

    KEIN Look-Ahead Bias: Wir nutzen nur vergangene Preise zur Berechnung.
    ...
    # EMAs berechnen (nur auf Basis vergangener Daten)
    ema_fast = pd.Series(closes).ewm(span=fast_period, adjust=False).mean().values
    ema_slow = pd.Series(closes).ewm(span=slow_period, adjust=False).mean().values

    targets_long = np.zeros(n)
    targets_short = np.zeros(n)

    for i in range(slow_period, n):
        # Fast EMA über Slow EMA = Bullish Momentum
        if ema_fast[i] > ema_slow[i] and ema_fast[i-1] <= ema_slow[i-1]:
            targets_long[i] = 1
        # Fast EMA unter Slow EMA = Bearish Momentum
...
def generate_breakout_labels(df, lookback=24, atr_mult=1.5):
    """
    Generiert Breakout-Labels ohne Look-Ahead Bias.

    Label = 1 wenn der aktuelle Close einen Breakout über/unter
    dem Lookback-High/Low darstellt.

    Args:
        df: DataFrame mit OHLC-Daten und _atr
        lookback: Perioden für High/Low-Berechnung
        atr_mult: ATR-Multiplikator für Breakout-Bestätigung
    ...
    lows = df["L"].values
    closes = df["C"].values
    atrs = df["_atr"].values if "_atr" in df.columns else np.ones(n) * 0.001

    for i in range(lookback + 1, n):
        # Lookback-Range (exkl. aktueller Bar)
        period_high = np.max(highs[i-lookback:i])
        period_low = np.min(lows[i-lookback:i])

        # Breakout über Period-High
        if closes[i] > period_high + atrs[i] * 0.5:
            targets_long[i] = 1

        # Breakout unter Period-Low
        elif closes[i] < period_low - atrs[i] * 0.5:
            targets_short[i] = 1

    return targets_long, targets_short


def generate_combined_labels(df, method="trend", **kwargs):
    """
    Wrapper-Funktion für verschiedene Label-Methoden.

    Args:
        df: DataFrame mit OHLC und Indikatoren
        method: "trend", "mean_reversion", "momentum", "breakout"
    ...
    """
    if method == "trend":
        return generate_trend_labels(df, **kwargs)
    elif method == "mean_reversion":
        return generate_mean_reversion_labels(df, **kwargs)
    elif method == "momentum":
        return generate_momentum_labels(df, **kwargs)
    elif method == "breakout":
        return generate_breakout_labels(df, **kwargs)
    else:

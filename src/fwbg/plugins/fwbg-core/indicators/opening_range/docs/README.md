...
| `sessions` | list[int] | `[0, 8, 13, 14]` | UTC hours for session-specific ORB features. Default: 0 (Asia/Tokyo), 8 (London), 13 (NY pre-market), 14 (NY open). Each session produces 5 features that persist until the next occurrence of that session hour. Range per element: 0-23. For strategies covering European or US markets with DST, include adjacent hours to capture both winter and summer UTC offsets (e.g. `[6, 7, 8]` covers DAX 08:00 at both CET=07 UTC and CEST=06 UTC). |
...
- **Session hours are UTC**: All datasources must provide UTC-indexed data. The `sessions` parameter specifies UTC hours directly. Because European markets observe DST (CET = UTC+1 in winter, CEST = UTC+2 in summer), a single local-time open maps to two different UTC hours depending on the season — e.g. DAX 08:00 CET = 07:00 UTC (winter) and 08:00 CEST = 06:00 UTC (summer). Include both adjacent hours to cover the full year: `[6, 7]` for the DAX 08:00 open. Asian markets (JP225, HK50) have no DST and always map to the same UTC hour. The recommended session array for a global strategy covering Asian, European and US opens is `[0, 1, 2, 6, 7, 8, 12, 13, 14]` UTC, which covers: 00-02 (JP225/HK50 opens), 06-08 (EU 08:00-09:00 CET/CEST), 12-14 (US pre-market and open 14:00-15:30 CET/CEST).
